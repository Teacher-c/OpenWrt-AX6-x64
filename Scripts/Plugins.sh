...
export CORE_TYPE=$(echo "$OWRT_TARGET" | grep -Eiq "cr6608|AC2100" && echo "mipsle-softfloat" || echo "$OWRT_TARGET" | grep -Eiq "64|86" && echo "amd64" || echo "$OWRT_TARGET" | grep -Eiq "Redmi-AX6" && "arm64")
